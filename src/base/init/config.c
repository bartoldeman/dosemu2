...
void dump_config_status(void (*printfunc)(const char *, ...))
{
    ...
    switch (config.speaker) {
      ...
      case SPKR_GENERATED: s = "generated"; break;
...
static void config_post_process(void)
{
    ...
    if (config.cpu_vm_dpmi == CPUVM_NATIVE && !config.dpmi_remote) {
      ...
      config.cpu_vm_dpmi = -1;
      config.dpmi_remote = 0;
#endif
    }
...
usage(char *basename)
{
	...
	"    -f set user config-file (dosemurc)\n"
	"    --Fusers bypass /etc/dosemu.users (^^)\n"
	"    --Flibdir change keymap and FreeDOS location\n"
	"    --Flocal_dir change location of dosemu local dir (~/.dosemu)\n"
	"    --Fimagedir bypass systemwide boot path\n"
	"    --Fdrive_c set drive C path\n"
