...
/*
 * A signal barrier (__atomic_signal_fence) forces all pending local memory
 * ops to be observed before a SIGSEGV is delivered to the *same* thread.
 * In practice this is exactly the same as barrier(), but since we have the
 * correct builtin, use it.
 *
 * SEQ_CST is weaker than the older __sync_* builtins and Linux
 * kernel read-modify-write atomics.  Signal barriers to obtain
 * the same semantics.
*
...
# define qatomic_set_mb(ptr, i) \
    ({ (void)qatomic_xchg(ptr, i); __atomic_signal_fence(__ATOMIC_SEQ_CST); })
#else
 ...
 * Derived from QEMU, utils/interval-tree.c
 */
